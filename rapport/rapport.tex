...
%\doublespacing 
	...
	L'ordonnanceur est la première fonction appelée par le programme. Il exécute la fonction selon la volonté de l'utilisateur. Nous avons choisit de d'abord rechercher les fichiers en input et en output et ensuite on parcourt les paramètres qui suivent la commande et dès qu'il s'agit d'une option valable, on exécute la fonction associée. Lors qu'il y a plus d'inputs que d'actions demandées, on choisit d'appliquer la dernière option appelée.
	\section{Gestion de fichiers}
	\subsection{Charger}
	Pour charger une image en mémoire, on a choisit de créer une structure qui prend en compte le type, la longueur, la largeur, la teinte max et la matrice des pixels. Étant donné que l'image pixmap est en fait qu'un fichier texte, il est très facile de lire les données. Ainsi on parcourt le fichier petit à petit avec \textbf{fscanf} et on crée ensuite notre Image.
	\subsection{Sauvegarder}
	Pour sauvegarder l'image il suffit d'écrire dans un fichier les différents éléments de la structure dans l'ordre imposé par le format pixmap.
	
	\section{Ce qu'il reste à faire}
		    Cependant, tout le code du second livrable ainsi que celui du panorama entier reste à faire même si la réflexion sur le code a été importante au cours de ce premier mois de travail. Plusieurs recherches ont été faites pour comprendre les fonctions à coder pour le livrable 2, notamment la convolution\cite{convolution}, qui est un produit matriciel entre une matrice de pixels et une matrice en paramètre dans un fichier. Cela fonctionne un peu comme un filtre appliqué à une image. Il suffit ensuite de mettre en place le code dans le programme. De plus il nous faut coder les fonction dilate et erode.\cite{ostermiller}\\
	\chapter{Les différentes façons de faire un panorama}
		...
		\section{Première méthode : Point par point\cite{wallace}}
		    %Quentin
		...
		\section{Troisième méthode : Méthode des coins}
		    ...
		    Ainsi, on reçoit un tableau de valeurs, ici 16, d'échelle de gris. On transforme ensuite le tableau de la manière suivante :
		    \begin{itemize}
		     \item si la valeur est inférieure à la valeur de seuil inférieur, on mets -1 dans le tableau ;
		     \item si la valeur est entre les deux valeurs de seuil, on lui assigne 0 ;
		     \item si la valeur est supérieure à la valeur de seuil supérieure, on lui assigne 1.
		    \end{itemize}
		    En choisissant plusieurs points de la sorte, on cherche alors dans l'autre image une correspondance entre certains pixels. Si il y a plusieurs identifications, on assemble les deux images de manière à ce qu'elles coïncident.
		\section{Post-assemblage}
		    En fin d'assemblage, on obtient deux images accolées. Pour remplir le reste de l'image résultat, on y met des pixels gris. On considère ce résultat comme une image source pour réutiliser l'algorithme d'assemblage de deux images si il reste des images non utilisée pour réaliser le panorama. Si aucune correspondance n'est trouvée entre les deux images, on les conserve et on utilise les autres images pour essayer de reconstituer une image susceptible de trouver des correspondances avec les images non-utilisées.
	\chapter*{Conclusion}
	    %Aucune idée
