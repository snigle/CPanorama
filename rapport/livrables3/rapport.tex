...
\chapter{Notre algorithme d'automatisation d'un panorama}
		...
		\section{Récupération des points clé}
			...
			Entre temps, nous avons trouvé un autre moyen pour trouver des points clé, qui utilisait les fonctions expliquées précédemment. 
			Pour cela, à partir d'une image en couleur, il faut la passer en greyscalepuis effectuer une convolution avec un filtre de Prewitt. Cela permet de détecter les contours tout en limitant l'effet du bruit sur la photo.
			\newline
			filtre de Prewitt : 
				\begin{tabular}{lll}
				   1 & 1 & 1 \\
		...
		\vspace{1,5cm}	
			...
			Ensuite, on converti cette image en binaire avec un seuil de 100. Enfin, on opère à une dilatation puis une érosion cinq fois de suite pour enlever les points esseulés. 
		...
		Pour trouver les points clés, nous avons d'abord fait beaucoup de recherches sur internet. Une des méthodes citées était le ZNCC, un algorithme qui donne une note entre deux points. Il suffisait donc avec cette méthode de comparer cette note. Cependant nous l'avons testé et il y avait trop de similitude entre les points. La deuxième méthode trouvée était de comparer les valeurs de Harris. Or harris ne nous satisfaisait pas pour la recherche des points clés donc nous avons abandonné. Finallement nous avons arrêté nos recherches sur internet pour se mettre à réfléchir de nous même et à concevoir un algorithme simple pour comparer nos points clés.
		
		\subsection{Schématisation de notre algorithme}
		Notre point de départ était de comparer des vecteurs dans chaque image. Nous avions d'abord essayé de comparer un seul vecteur sur des images coupé à l'ordinateur et le résultat était très satisfaisant. Par contre quand nous avons essayé cette méthode sur des photos cela n'a pas été concluant.
		...
		De loin, le résultat n'est pas trop mal mais lorsqu'on se rapproche, on distingue clairement que si on aligne une partie de l'image, l'autre partie est complètement décallée. Ce phénomène est dû à la distortion de l'appareil photo. Nous avons donc recherché un moyen de corriger cette distortion. Pour cela nous avons utilisé la correction de Lens.
		%http://www.paulbourke.net/miscellaneous/lenscorrection/%.ous 
	%screenshot%
	
	%Algorithme de Lens%
	
	Nous avons testé cet algorithme sur plusieurs tirages et il corrigeait très bien cette distortion. Pour vérifier cela nous avons de nouveau coller nos images après la correction avec Gimp et les images s'assemblaient parfaitement.
		
		\subsubsection{Comparaison des vecteurs}
		
		
		\paragraph{Sélection des vecteurs\\}
		Une fois que les images étaient prêtes, nous avons pu réfléchir à comment comparer les vecteurs. Nous vons choisit de sélectionner 20 vecteurs de même origine dans une fênetre de 30px sur 30px. Nous avons donc parcourue la première image et pour chaque point clé nous avons générer un tableau de 20 points proche de ce point clé. Avec ce tableau nous avons pu calculer nos 20 vecteurs.
...
\end{lstlisting}
		...
		Le but de notre programme était de prendre en paramètre un certains nombre de photo et de les assembler en fonction de leurs correspondances. Nous devions trouver un moyen de savoir la position de chaque image entre elle. La méthode utilisée est de calculer tout les décalages possibles entre chaque image avec notre algorithme de comparaison et de prendre celui qui apparait le plus de fois. Pour chaque image nous avons donc son meilleur décalage associé à une image dans une certaine direction.
\\\\
\begin{lstlisting}
Fonction panorama
	charger les images en mémoires
	charger les matrices des points clés
	sélectionner des bandes pour chaque MatricePointClé
	calculer tout les décalages possibles pour chaque image
	coller les images en fonction des décalages
	sauvegarder le résultat
\end{lstlisting}
		
		\section{Comment améliorer cet algorithme ?}%Ludo
		
		Améliorer l'algorithme pour coller toutes les images.
		Améliorer la sélection des points clés pour en avoir moins et gagner du temps pour la comparaison.
